...
class PodsumowanieZadanTestCase(TestCase):
    def setUp(self):
    ...
    def test_mix_model_commission_task(self):
        ...
        zadanie = Zadanie.objects.create(
            nazwa="T1",
            prog_1=5,
            prog_1_premia=Decimal("100"),
            prog_2=7,
            prog_2_premia=Decimal("120"),
            data_start=self.today,
            data_koniec=self.today,
        )
        zadanie.produkty.set([p1, p2])
        ...
        response = self.client.get(
            reverse("produkty:podsumowanie_sprzedazy")
        )
    ...
    def test_mix_model_multiplier_task(self):
        ...
        zadanie = Zadanie.objects.create(
            nazwa="Mult",
            mnoznik_mix=Decimal("2"),
            data_start=self.today,
            data_koniec=self.today,
        )
        zadanie.produkty.add(p)
        ...
        self.assertEqual(entry["stawka"], Decimal("10"))
        self.assertEqual(entry["suma_prowizji"], Decimal("20"))
    ...
    def test_specific_models_task(self):
        ...
        self.assertEqual(response.context["task_rewards"][0]["premia"], Decimal("15"))
    ...
    def test_mix_prowizja_task_threshold(self):
        ...
        zadanie = Zadanie.objects.create(
            nazwa="MixProv",
            typ=Zadanie.Typ.MIX_PROWIZJA,
            prog_mix=5,
            prog_1=5,
            prog_1_premia=Decimal("100"),
            prog_2=7,
            prog_2_premia=Decimal("140"),
            data_start=self.today,
            data_koniec=self.today,
        )
        zadanie.produkty.set([p1, p2])
    ...
    def test_mix_mnoznik_task_threshold(self):
        ...
        self.assertEqual(entry1["stawka"], Decimal("10"))
        self.assertEqual(entry1["suma_prowizji"], Decimal("10"))
        self.assertEqual(entry2["stawka"], Decimal("20"))
        self.assertEqual(entry2["suma_prowizji"], Decimal("40"))


@override_settings(
    DATABASES={"default": {"ENGINE": "django.db.backends.sqlite3", "NAME": ":memory:"}}
)
class ZadanieTypeTestCase(TestCase):
    def setUp(self):
    ...
    def test_create_mix_prowizja_task(self):
        response = self.client.post(
            reverse("produkty:nowe_zadanie"),
            {
                ...
                "data_koniec": "2024-01-31",
            },
        )
        self.assertEqual(response.status_code, 302)
        zadanie = Zadanie.objects.get(nazwa="Mix prowizja")
        self.assertEqual(zadanie.typ, Zadanie.Typ.MIX_PROWIZJA)
        self.assertEqual(zadanie.prog_mix, 5)
    ...
    def test_create_mix_mnoznik_task(self):
        ...
        self.assertEqual(response.status_code, 302)
        zadanie = Zadanie.objects.get(nazwa="Mix mnoznik")
        self.assertEqual(zadanie.typ, Zadanie.Typ.MIX_MNOZNIK)
        self.assertEqual(zadanie.mnoznik_mix, Decimal("1.5"))
